...
static int CmdHf14AConfig(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf 14a config",
                  ...
                  "hf 14a config --mag enable      -> Enable Apple magsafe polling\n"
                  "hf 14a config --mag disable     -> Disable Apple magsafe polling\n"
                  "hf 14a config --pla <hex>       -> Set polling loop annotation (max 22 bytes)\n"
                  "hf 14a config --pla disable     -> Disable polling loop annotation\n");
    void *argtable[] = {
        arg_param_begin,
        ...
        arg_str0(NULL, "mag", "<enable|disable>", "Configure Apple MagSafe polling"),
        arg_str0(NULL, "pla", "<hex|disable>", "Configure polling loop annotation"),
    ...
    if (vlen > 0) {
        if (strcmp(value, "std") == 0) magsafe = 0;
            ...
            PrintAndLogEx(ERR, "magsafe argument must be 'std', 'skip', 'disable' or 'enable'");
            CLIParserFree(ctx);
            return PM3_EINVARG;
        }
    }
